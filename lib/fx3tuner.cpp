...
void Fx3Tuner::SetFreqDivs(int pll_idx, uint32_t N, uint32_t R)
{
    ...
    pause();
    dev->send8bitSPI( regHi, addrHi );
    pause(5);
    dev->send8bitSPI( regLo, addrLo );
    pause();
}
...
void Fx3Tuner::ConfigureClockLVDS(int clk_src, int clk_div, int ampl, int outDC) {
    ...
    dev->send8bitSPI( ( div_bit << 0 ), 11 );
    dev->send8bitSPI( (src_bit<<5)|(type_bit<<4)|(cc_bit<<2)|(ol_bit<<0), 12 );
    pause();
}
...
void Fx3Tuner::Set_AGC() {
    ...
    for ( int ch = 0; ch < 4; ch++ ) {
        dev->send8bitSPI( reg15, 15 + ch*7 );
        dev->send8bitSPI( reg16, 16 + ch*7 );
        dev->send8bitSPI( reg18, 18 + ch*7 );
    }
}
