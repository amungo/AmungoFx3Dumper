...
fx3_dev_err_t FX3DevIfce::init_fpga(const char* bitFileName)
{
    fx3_dev_err_t retCode = FX3_ERR_OK;

    FILE* fwfile = fopen( bitFileName, "r" );
    if( !fwfile ) {
    ...
    int fw_size8  = ftell( fwfile );
    fseek( fwfile, 0, SEEK_SET );

    // Read firmware
    char* fw_buf = new char[ fw_size8 ];
    fread( fw_buf, sizeof(char), fw_size8, fwfile );
    fclose( fwfile );
    ...
    if(retCode == FX3_ERR_OK)
    {
        ...
        {
           if((written + len) > fw_size8)
               len = fw_size8 - written;

           if(sendECP5((uint8_t*)tbuff, len) != FX3_ERR_OK)
           {
