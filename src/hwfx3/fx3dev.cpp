...
FX3Dev::~FX3Dev() {
    ...
    if ( resetECP5() == FX3_ERR_OK) {
        ...
        fprintf( stderr, "__error__ LATTICE SWITCH OFF failed\n" );
    }
...
fx3_dev_err_t FX3Dev::getReceiverRegValue(uint8_t addr, uint8_t &value) {
    readFromDeviceSynch(addr, &value);
    return FX3_ERR_OK;
}
...
void FX3Dev::changeHandler(DeviceDataHandlerIfce *handler) {
...
void LIBUSB_CALL FX3Dev::onDataReady( libusb_transfer* xfr ) {
    ...
    switch(xfr->status) {
        case LIBUSB_TRANSFER_COMPLETED:
            ...
            {
                std::lock_guard<std::mutex> guard(self->handler_mtx);
